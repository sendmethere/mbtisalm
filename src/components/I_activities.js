...
function I_activities() {
  const { pass, setPass, sceneIndex, setSceneIndex, activityCompleted, setActivityCompleted } = useStore();

  const handleJoinActivity = (event) => {
    if (event.target.getAttribute('activity') === null) {
      return;
    }
    const sceneTo = parseInt(event.target.getAttribute('sceneTo'));
    const activity = parseInt(event.target.getAttribute('activity'));
  ...
  return (
    <div className='flex'>
      <div className='grow bg-red-50 text-red-200 hover-zoom hover-light p-4 rounded-2xl m-1 text-center'
        activity='1'
          sceneTo='9'
      ...
      <div className='grow bg-yellow-50 text-yellow-200 hover-zoom hover-light p-4 rounded-2xl m-1 text-center'
        activity='2'
          sceneTo='10'
          onClick={handleJoinActivity}>
      ...
      <div className='grow bg-green-50 text-green-200 hover-zoom hover-light p-4 rounded-2xl m-1 text-center'
          activity='3'
          sceneTo='11'
          onClick={handleJoinActivity}>
